...
import { RPC_URL, PROGRAM_ID, UNIVERSE_NAME, TIMER_MINT } from "../app/lib/constants";
...
async function main() {
  ...
  console.log('Current game count:', gameIndex);
  ...
  // Lock time: 1 hour from now, settle: 2 hours from now
  const now = Math.floor(Date.now() / 1000);
  const lockTime = now + 3600;
  const settleTime = now + 7200;

  await program.methods
    .createGame({ assetPrice: { assetSymbol: Array.from(Buffer.from('sol'.padEnd(8, '\0'))), targetPrice: new anchor.BN(200), direction: { above: {} } } }, new anchor.BN(lockTime), new anchor.BN(settleTime))
    .accounts({
      game: gamePda,
