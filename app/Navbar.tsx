    ...
    <nav className="flex items-center justify-between px-6 py-4 border-b border-gray-800 bg-gray-950">
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-gray-950 border-b border-gray-800 px-6 py-4 flex flex-col gap-4 sm:hidden z-50">
          <Link href="/faq" onClick={() => setMenuOpen(false)} className="text-gray-400 hover:text-white text-sm transition-colors">
            FAQ
          </Link>
          <Link href="/whitepaper" onClick={() => setMenuOpen(false)} className="text-gray-400 hover:text-white text-sm transition-colors">
            Whitepaper
          </Link>
        </div>
      )}
